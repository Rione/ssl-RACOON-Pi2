...
Release の tar には Go バイナリに加え、`camera/` パッケージ一式（Python ソース・`requirements.txt`・YOLO キャリブ用モデル `camera/yolo/last.pt` / `best.pt`）が同梱されます（モデルは Release ビルド時に submodule を取得して同梱）。展開するとバイナリと同じディレクトリに `camera/` が置かれるため、別端末でも `git clone` なしで動かせます（初回のみ `pip install -r camera/requirements.txt` は必要）。

同一バージョンタグ（例: `v1.0.0`）の Release に両方のアセットが含まれますが、実行中のビルドに応じて正しい方のみが選ばれます。自動アップデート時は**バイナリの差し替えに加え、tar 内の `camera/`（Python コード・モデル）もバイナリと同じディレクトリへ展開**して同期します（`camera/` 配下以外は上書きしません）。
